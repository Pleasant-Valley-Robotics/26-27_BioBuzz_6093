...
@Config
@Autonomous(name = "Close 15", group = "Close")
public class CloseDIOAuto15Ball extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        Actions.runBlocking(drive.actionBuilder(drive.localizer.getPose())
                        .turnTo(Math.toRadians(intakeAngle))
                .strafeTo(new NewVector(-20.8829, 15.6723 * PoseStorage.isRed).getVector2d())
                .strafeTo(Positions.diagIntakeMid.getVector2d()).build());
        ...
        Actions.runBlocking(new RaceAction(
                drive.actionBuilder(drive.localizer.getPose())
                        .turnTo(Math.toRadians(-90 * PoseStorage.isRed))
                        .splineTo(Positions.shootPose.getVector2d(),Math.toRadians(shootAngle)).build(),
                readContinous()
        ));
        ...
        turntable.turnToPosition(1);
        Actions.runBlocking(drive.actionBuilder(drive.localizer.getPose())
                        .turnTo(Math.toRadians(121.4573 * PoseStorage.isRed))
                .strafeTo(Positions.diagIntakeMid.getVector2d())
                .strafeTo(Positions.gateIntake.getVector2d()).build());
        driveForward(.3, .4);
        ...
        Actions.runBlocking(drive.actionBuilder(drive.localizer.getPose())
                        .turnTo(Math.toRadians(Math.toRadians(-90 * PoseStorage.isRed)))
                        .splineTo(Positions.shootPose.getVector2d(), shootAngle)
    ...
    private void intakeBallsGate(double limitTime, double intakeSpeed) {
        intake.setPower(1);
        drive.rightBack.setPower(intakeSpeed);
        drive.rightFront.setPower(intakeSpeed);
        drive.leftBack.setPower(intakeSpeed);
